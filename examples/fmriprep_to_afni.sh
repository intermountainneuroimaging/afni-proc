...
function regressors_of_interest () {
files=($1)
# Loop through each file
for idx in "${!files[@]}"; do
  #file from list
...
do
    ...
    rot_x_deg=$(awk '{print $1 * 180 / 3.141593}' <<< $rot_x)
    rot_y_deg=$(awk '{print $1 * 180 / 3.141593}' <<< $rot_y)
    rot_z_deg=$(awk '{print $1 * 180 / 3.141593}' <<< $rot_z)

    # Reorder columns and output the result
    echo "$rot_z_deg $rot_x_deg $rot_y_deg $trans_z $trans_x $trans_y" >> $file.bak
...
ls $filepath_regex
regressors_of_interest "$(ls $filepath_regex)"
...
# delete intermediate files
rm ${filepath_regex//.tsv/.nr.csv}
rm ${filepath_regex//.tsv/.motion.csv}
...
echo "$EVENTNAME Pipeline succeeded."
fi
