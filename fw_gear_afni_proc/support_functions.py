...
def cleanup(gear_options: dict, app_options: dict):
    ...
    outfiles = [x for x in new if x not in set(gear_options["unzipped-files"])]

    outfiles_rel = [i.replace(str(gear_options["work-dir"]) + os.sep, '') for i in outfiles]

    with open(os.path.join(str(gear_options["work-dir"]), "files.txt"), 'w') as f:
        f.write("\n".join(map(str, outfiles_rel)))

    # use new list for final output
    # zip output files
    os.chdir(gear_options["work-dir"])
    output_zipname = gear_options["output-dir"].absolute().as_posix() + "/afni_" + \
    ...
    os.chdir(gear_options["output-dir"])
    duResults = sp.Popen(
...
def searchfiles(path, dryrun=False, exit_on_errors=True):
    cmd = "ls -d " + path
